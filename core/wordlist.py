...
class WordlistManager:
    ...
    def _save_local_wordlist(self, vocabulary_id: str, name: str, words: List[HotWord]) -> None:
        ...
        with open(os.path.join(self.wordlist_path, f"{vocabulary_id}.json"), 'w', encoding='utf-8') as f:
    ...
    def load_local_wordlists(self) -> Dict[str, Dict]:
        ...
        for filename in os.listdir(self.wordlist_path):
            if filename.endswith('.json') and filename != "saved_vocabulary_ids.json":
                try:
                    with open(os.path.join(self.wordlist_path, filename), 'r', encoding='utf-8') as f:
                        wordlist = json.load(f)
                        vocabulary_id = wordlist.get('vocabulary_id')
                        if vocabulary_id:
                            wordlists[vocabulary_id] = wordlist
                except Exception as e:
