...
class DimensionEditor:
    ...
    def __init__(self, initial_dimensions: Dict = None):
        ...
        if initial_dimensions and isinstance(initial_dimensions, dict):
            ...
            if 'initial key dimensions' in st.session_state.templates:
                ...
                for dim2 in template_data:
                    # 兼容 value 为 list 的情况
                    if isinstance(template_data[dim2], dict):
                        self.dimensions['level3'][dim2] = list(template_data[dim2].keys())
                    elif isinstance(template_data[dim2], list):
                        self.dimensions['level3'][dim2] = template_data[dim2]
                    else:
        ...
        if 'dimension_state' not in st.session_state:
            st.session_state.dimension_state = {
                'expanded_level2': self.dimensions['level2'].copy(),  # 初始展开所有二级维度
    ...
    def _load_default_templates(self):
        ...
        try:
            # 使用绝对路径构建模板目录路径
            template_dir = '/Users/apple/Desktop/AI video/AI-Video-Master_v0.9/data/dimensions/'
                    ...
                    if filename.endswith('.json'):
                        ...
                        try:
                            with open(template_path, 'r', encoding='utf-8') as f:
                                template_data = json.load(f)  # This line was already correct
    ...
    def render(self):
        ...
        if self.dimensions['level2']:
            ...
            for i, dim2 in enumerate(self.dimensions['level2']):
                ...
                # 自动展开所有维度层级（移除折叠功能）
                if dim2 not in st.session_state.dimension_state['expanded_level2']:
    ...
    def apply_template(self, template_data: Dict):
        ...
        for dim2, level3_dict in template_data.items():
            ...
            # 兼容 value 为 list、dict、str 等情况
            if isinstance(level3_dict, dict):
                self.dimensions['level3'][dim2] = list(level3_dict.keys())
            elif isinstance(level3_dict, list):
                self.dimensions['level3'][dim2] = level3_dict
            else:
                self.dimensions['level3'][dim2] = []
