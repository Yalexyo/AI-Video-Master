...
def main():
    ...
    if 'settings' in st.session_state and not st.session_state.settings.get('dimensions'):
        try:
            # 使用当前项目目录的路径
            template_path = os.path.join('/Users/apple/Desktop/AI video/AI-Video-Master_v0.9/data/dimensions', 'initial_key_dimensions.json')
            if os.path.exists(template_path):
                ...
                for dim2 in initial_template:
                    # 兼容 value 为 dict、list、str 等
                    if isinstance(initial_template[dim2], dict):
                        initial_dimensions['level3'][dim2] = list(initial_template[dim2].keys())
                    elif isinstance(initial_template[dim2], list):
                        initial_dimensions['level3'][dim2] = initial_template[dim2]
                    else:
                        initial_dimensions['level3'][dim2] = []
...
def show_analysis_page():
    ...
    valid_urls = [url for url in st.session_state.settings.get('urls', []) 
                 if url.startswith(('http://', 'https://')) and url.lower().endswith(('.mp4', '.mov'))]
    if url_count > 0:
...
def import_urls_from_csv(uploaded_file):
    ...
    """
    try:
        # 读取CSV文件（支持两种格式：单列URL格式和object,url双列格式）
        try:
            # 先尝试读取带header的双列格式
